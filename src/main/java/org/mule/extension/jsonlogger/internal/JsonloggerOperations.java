                                        ...
                                        if (config.getJsonOutput().isParseContentFieldsInJsonOutput()) {
                                            ...
                                            if (typedVal.getDataType().getMediaType().getPrimaryType().equals("application") && typedVal.getDataType().getMediaType().getSubType().equals("json")) {
                                                ...
                                                if (!dataMaskingFields.isEmpty()) {
                                                	...
                                                	if(k.toString().equals("attributes")) {
                                                		JsonNode tempContentNode = om.getObjectMapper().readTree((InputStream)typedVal.getValue());
                                                		if (tempContentNode != null) {
                                                			JsonNode attributesNode = tempContentNode.path("attributes");
                                                			if (attributesNode != null) {
                                                				ObjectNode attNode = om.getObjectMapper().createObjectNode();
                                                				JsonNode headersNode = attributesNode.path("headers");
                                                				if (headersNode != null) {
                                                					attNode.set("headers", headersNode);
                                                					//typedValuesAsJsonNode.put("headers", headersNode);
                                                				}
                                                				JsonNode queryParamsNode = attributesNode.path("queryParams");
                                                				if (queryParamsNode != null) {
                                                					attNode.set("queryParams", queryParamsNode);
                                                					//typedValuesAsJsonNode.put("queryParams", queryParamsNode);
                                                				}
                                                				typedValuesAsJsonNode.put(k,attNode);
                                                			}
                                                		}
                                                	} else {
